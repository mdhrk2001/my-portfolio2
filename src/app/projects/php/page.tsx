...
import { phpProjects } from "../../../data/projectData";
import { SiHtml5, SiCss3, SiJavascript, SiPhp, SiMysql } from "react-icons/si";  // Import Simple Icons
...
function ProjectCard({ title, description, link, techStack }: { title: string; description: string; link?: string; techStack: string[] }) {
  ...
  const renderTechIcons = () => {
    return techStack.map((tech, index) => {
      switch (tech) {
        case "HTML5":
          return <SiHtml5 key={index} className="w-7 h-7 text-[#E34F26]" />;
        case "CSS3":
          return <SiCss3 key={index} className="w-7 h-7 text-[#1572B6]" />;
        case "JavaScript":
          return <SiJavascript key={index} className="w-7 h-7 text-[#F7DF1E]" />;
        case "PHP":
          return <SiPhp key={index} className="w-7 h-7 text-[#777BB4]" />;
        case "MySQL":
          return <SiMysql key={index} className="w-7 h-7 text-[#4479A1]" />;
        default:
          return null;
      }
    });

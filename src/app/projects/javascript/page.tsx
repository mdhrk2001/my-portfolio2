...
import { javaScriptProjects } from "../../../data/projectData";
import { SiReact, SiNodedotjs, SiExpress, SiMongodb, SiFirebase } from "react-icons/si";  // Import Simple Icons
  ...
  function ProjectCard({ title, description, link, techStack }: { title: string; description: string; link?: string; techStack: string[] }) {
    ...
    const renderTechIcons = () => {
      return techStack.map((tech, index) => {
        switch (tech) {
          case "Reactjs":
            return <SiReact key={index} className="text-[#61DAFB] w-7 h-7" />;
          case "Nodejs":
            return <SiNodedotjs key={index} className="text-[#339933] w-7 h-7" />;
          case "Expressjs":
            return <SiExpress key={index} className="text-black dark:text-white w-7 h-7" />;
          case "Mongodb":
            return <SiMongodb key={index} className="text-[#47A248] w-7 h-7" />;
          case "Firebase":
            return <SiFirebase key={index} className="text-[#FFCA28] w-7 h-7" />;          
          default:
            return null;
        }
      });
